...
- Управляемое обновление установленных файлов с разбором конфликтов (текущая установка сохраняет изменённое и останавливается на конфликте)
- Bilingual README (EN)

## [1.1.0] - 2026-09-11

### Добавлено
- Общая методология для Claude Code и Codex с отдельными входами `CLAUDE.md` и `AGENTS.md`.
- Консервативный установщик Python: предварительный просмотр, явное применение, сохранение существующих правил и настроек, повторный запуск без дублей.
- Автоматические тесты установки и проверок подготовленного содержимого Git.

### Изменено
- Адаптивный старт: без идеи, с идеей, с готовой архитектурой, с кодом или продолжение работы. Без обязательного повторного интервью.
- Один источник решений и общий план версии, самостоятельное выполнение согласованных фаз, независимая проверка и исправления, сохранение состояния и границы доступа.
- Проверка секретов читает индекс Git, поддерживает сложные имена файлов, не печатает значения находок и не исключает целиком scripts/hooks.
- Уточнены ограничения безопасности, браузерных тестов и выпуска; исправлена ссылка на официальный пакет Playwright MCP.

### Границы
- Установщик не устанавливает сами Claude Code/Codex, не меняет их разрешения, не подключает MCP и не публикует проект.
- Поддержка входных файлов не является доказательством полного автономного запуска обеих моделей. Такой запуск зависит от выбранного инструмента, среды и доступов.
- Сканер эвристический и не заменяет изоляцию, отдельную проверку истории и защиту секретов на уровне инфраструктуры.
