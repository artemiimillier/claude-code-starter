...
> Запуск, остановка, бэкап БД, AI-коммит и прочие повседневные команды — в [ADMIN.md](ADMIN.md) и `./admin.sh`. Если что-то «не запускается» — сначала попробуй `./admin.sh stack-up` и `./admin.sh status`.
